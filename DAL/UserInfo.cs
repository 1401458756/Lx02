...
namespace DAL
{
    public class UserInfo
    {
        ...
        public Model.UserInfo GetModel(string UserName)
        {
            using (IDbconnection cn=new MySqlConnection(cns))
            {
                string sql = "select * from userifo where sername=@username";
                return cn.QueryFirstOrDefault<GetModel.UserInfo>(sql, new { username = UserName });
            }
        }
        ...
        public IEnumerable<Model.UserInfo> Getpage(Model.Page page)
        {
            using (IDbConnection cn=new MySqlConnection(cns))
            {
                ...
                return cn.Query<GetModel.UserINfoNo>(sql, page);
            }
